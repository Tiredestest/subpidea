-- Existing stories remain main stories; event reviews use the same chapter permissions.
alter table public.story_arcs add column story_kind text not null default 'main' check (story_kind in ('main','event'));

create or replace function public.admin_apply_changes(operations jsonb) returns integer
...
begin
 ...
 for op in select value from jsonb_array_elements(operations) loop
  ...
  allowed:=case tab
   ...
   when 'story_arcs' then array['story_kind','game_id','source_id','slug','title','summary','cover_image','detail_image','sort_order','is_published','release_date_kr','release_date_jp']
